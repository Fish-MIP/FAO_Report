...
main <- ggplot()+
  ...
  geom_sf(data = eez_proj, aes(fill = contnnt))+
...
pdf("figures/fao_eez_global_map.pdf", width=14, height= 9)
main
dev.off()
...
pdf("figures/global_perc_change_map_50-00.pdf", width=14, height= 9)
all_plots
dev.off()
```
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                                  ...
                                  oob = oob_squish),
                new_scale_fill(),
                geom_sf(inherit.aes = F, data = eez[eez$contnnt == "Asia",], 
                         fill = NA, colour = "#5b5b5b", show.legend = F,
                         linewidth = 0.25),
                geom_sf(inherit.aes = F, 
                        data = world[world$CONTCD == "ASI",], 
...
pdf("figures/asia_perc_change_map_50-00.pdf", width=14, height= 9)
asia_plots
dev.off()
...
pdf("figures/americas_perc_change_map_50-00.pdf", width=14, height= 9)
americas_plots
dev.off()
...
#Split EEZ in two hemispheres
eez_east <- st_crop(eez[eez$contnnt == "Europe",], 
                    st_bbox(c(xmin = -50, ymin = 20, xmax = 180, ymax = 90)))
eez_west <- st_crop(eez[eez$contnnt == "Europe",], 
...
pdf("figures/europe_perc_change_map_50-00.pdf", width=14, height= 9)
europe_plots
dev.off()
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8, 
                                  ...
                                  oob = oob_squish),
                new_scale_fill(),
                geom_sf(inherit.aes = F, data = eez[eez$contnnt == "Africa",], 
                         fill = NA, colour = "#5b5b5b", show.legend = F,
                         linewidth = 0.25),
                geom_sf(inherit.aes = F, 
                        data = world[world$CONTCD == "AFR",], 
                        color = NA, aes(fill = class_fish), show.legend = F),
                geom_sf(inherit.aes = F, 
                        data = world[world$CONTCD != "AFR",], 
                        color = "#dbdade", fill = "#dbdade"),
...
pdf("figures/africa_perc_change_map_50-00.pdf", width=14, height= 9)
africa_plots
dev.off()
```
...
pdf("figures/oceania_perc_change_map_50-00.pdf", width=14, height= 9)
oceania_plots
dev.off()
```
...
pdf("figures/fao_regions_perc_change_map_50-00.pdf", width=14, height= 9)
fao_reg_plots
dev.off()
...
fao_fishereies_plot<-ggplot()+
  ...
  geom_sf(data = world_proj, color = "#dbdade", fill = "#dbdade")+
  #Adding coastlines and international boundaries
  geom_sf(data = world_bound, lwd = 0.25, color = "#484848", show.legend = F, 
          aes(linetype = as.factor(TYPE)))+
  scale_linetype_manual(values = c("0" = "solid", "1" = "twodash", 
                                   "2" = "dotdash", "3" = "dashed", 
                                   "4" = "dotted"))+
...
pdf("figures/fao_regions_perc_sustainable_fisheries.pdf", width=14, height= 9)
fao_fishereies_plot
dev.off()
```
...
# work around: 
legend = cowplot::get_plot_component(p00_585, 'guide-box-bottom', return_all = TRUE)
...
pdf("figures/global_sd_map_50-00.pdf", width=14, height= 9)
all_plots
dev.off()
...
```{r}
# # Saving multi-panel plot
# ggsave(file.path(out_folder, "global_cv_map_50-00.pdf"), device = "pdf",
#        width = 14, height = 9)

# jpeg("figures/global_cv_map_50-00.jpeg")
# all_plots
# dev.off()

pdf("figures/global_cv_map_50-00.pdf", width=14, height= 9)
all_plots
dev.off()
...
# combine Maps of % change + SD + model agreement. 

# model agreement, only for rel_change_mean00_ssp585? 
# Listing all relevant files to calculate biomass projections
maps_data_agreement_proj  <- list.files(out_folder, pattern = "_perc_bio_change_data_map.csv", 
                        full.names = T) |> 
  map_df(~fread(.)) |> 
  bind_rows() |>
  # consider only relevant columns 
  select(x,y,mem,esm, rel_change_mean00_ssp585,rel_change_mean00_ssp126) %>%
  # wide format
  gather(scenario, value, -c(x,y,mem,esm)) %>% 
  #Calculations performed by grid cell
  group_by(x, y, scenario) %>% 
  summarise(n_model = n(), # to calculate agreement across models
            n_positive = sum(value>0, na.rm = T),
            n_negative = sum(value<0, na.rm = T), 
            same_sign = max(n_positive,n_negative),
            agreement = (same_sign/n_model)*100) %>% # 0 land to be converted into NAs 

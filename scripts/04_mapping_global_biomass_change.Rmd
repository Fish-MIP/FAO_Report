...
We will load the data to be plotted on land from Google Drive.  
  
```{r}
fisheries_catch <- drive_read_string("https://drive.google.com/file/d/1adH-7AbNJln9E9_vaGcyit-czMmkGndo/view?usp=drive_link") |> 
  #Read as csv and convert to data frame
  read_csv(col_select = c(C_Name, CCode, MarCap1014)) |> 
  #Remove duplicates
  distinct() |> 
  #Create new column with classification
  mutate(class_fish = cut(MarCap1014, 
                     breaks = quantile(fisheries_catch$MarCap1014,
                                       probs = seq(0, 1, length.out = 4)), 
                     labels = c("low", "medium", "high")))

#Now we can add this information to the world map by performing a data frame join
world_proj <- world_proj |> 
  left_join(fisheries_catch |> select(!C_Name), by = join_by("adm0_a3" == "CCode"))
```
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                ...
                new_scale_fill(),
                geom_sf(inherit.aes = F, data = world_proj, aes(fill = class_fish),
                        show.legend = F),
                scale_fill_brewer(palette = "Blues", na.value = "#bcbcbc"),
...
land <- ggplot()+
  ...
  scale_fill_brewer(palette = "Blues", na.value = "#bcbcbc", 
                    guides(title = "Yearly landings between 2005-2014"),
                    labels = c("low", "medium", "high", "no data"))+
...
pop_sum <- function(file){
  df <- read_csv(file, skip = 4) |> 
    ...
    pivot_longer(!c(iso, country), names_to = "year", values_to = "population") |> 
...
pop_change <- file_list |> 
  ...
  mutate(rel_change_mean50_ssp126 = ((mean50_ssp126-reference)/reference)*100,
         ...
         rel_change_mean00_ssp126 = ((mean00_ssp126-reference)/reference)*100,
         rel_change_mean00_ssp585 = ((mean00_ssp585-reference)/reference)*100) |> 
  #Matching ISO codes to world shapefile
  mutate(iso = case_when(country == "Western Sahara" ~ "SAH",
                         str_detect(country, "Palestine") ~ "PSX",
                         country == "South Sudan" ~ "SDS",
                         T ~ iso))

#Now we can add this information to the world map by performing a data frame join
world <- world |> 
  left_join(pop_change |> select(!country), by = join_by("adm0_a3" == "iso"))
```
...
```{r}
#Define base steps for maps
base_gg <- list(geom_sf(inherit.aes = F, data = eez[eez$region == "Asia",], 
...
#Plotting land masses to get legend
land <- ggplot()+
  geom_sf(data = world, aes(fill = rel_change_mean50_ssp126), colour = "#5b5b5b",
          linewidth = 0.25)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish,
                    guides(title = "% changes in human population"))+
  guides(fill = guide_colorbar(title.position = "top", title.hjust = 0.5, barwidth = 15))+
  theme(legend.position = "bottom")
...
#Plotting fish biomass to get legend
fish <- ggplot()+
  geom_tile(data = asia_data, aes(x, y, fill = rel_change_mean50_ssp126_mean))+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish,
                    guides(title = "% change in fish biomass"))+
...
p50_126 <- asia_data |> 
  ...
  geom_sf(inherit.aes = F, data = world, aes(fill = rel_change_mean50_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2041-2050")+
  base_gg
...
```{r}
#Define base steps for maps
base_gg <- list(geom_sf(inherit.aes = F, data = eez[eez$region == "Africa",], 
                        fill = NA, colour = "#5b5b5b",
                        show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
...
```{r}
#Define base steps for maps
base_gg <- list(geom_sf(inherit.aes = F, data = eez_euro, fill = NA, 
...
p50_126 <- europe_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = eurocent, aes(fill = rel_change_mean50_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2041-2050")+
...
p50_585 <- europe_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = eurocent, aes(fill = rel_change_mean50_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP5-8.5: 2041-2050")+
...
p00_126 <- europe_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = eurocent, aes(fill = rel_change_mean00_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2091-2100")+
...
p00_585 <- europe_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = eurocent, aes(fill = rel_change_mean00_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP5-8.5: 2091-2100")+
...
```{r}
#Define base steps for maps
base_gg <- list(geom_sf(inherit.aes = F, data = eez_amer, fill = NA, 
                        colour = "#5b5b5b", show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5),
...
p50_126 <- americas_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = amercent, aes(fill = rel_change_mean50_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2041-2050")+
...
p50_585 <- americas_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = amercent, aes(fill = rel_change_mean50_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP5-8.5: 2041-2050")+
...
p00_126 <- americas_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = amercent, aes(fill = rel_change_mean00_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2091-2100")+
...
p00_585 <- americas_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = amercent, aes(fill = rel_change_mean00_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP5-8.5: 2091-2100")+
...
```{r}
#Define base steps for maps
base_gg <- list(geom_sf(inherit.aes = F, data = eez_ocean, fill = NA, 
                        colour = "#5b5b5b", show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5),
...
p50_126 <- ocean_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = oceancent, aes(fill = rel_change_mean50_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2041-2050")+
...
p50_585 <- ocean_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = oceancent, aes(fill = rel_change_mean50_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP5-8.5: 2041-2050")+
...
p00_126 <- ocean_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = oceancent, aes(fill = rel_change_mean00_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP1-2.6: 2091-2100")+
...
p00_585 <- ocean_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  new_scale_fill()+
  geom_sf(inherit.aes = F, data = oceancent, aes(fill = rel_change_mean00_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25, show.legend = F)+
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8, type = scale_fill_distiller, 
                    palette = "PRGn", oob = scales::oob_squish)+
  labs(title = "SSP5-8.5: 2091-2100")+
...
Finally, we will create maps for the FAO regions.  
  
```{r}
#Define base steps for maps
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                                  type = scale_fill_custom, oob = scales::oob_squish),
                geom_sf(inherit.aes = F, data = fao_proj, fill = NA, colour = "#5b5b5b",
                        show.legend = F, linewidth = 0.25),
...
p00_585 <- ggplot()+
  ...
  labs(title = "SSP5-8.5: 2091-2100")+
  base_gg+
  guides(fill = guide_colorbar(title = "% change in fish biomass",
...
#Plotting everything together
fao_reg_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, nrow = 1,
                                        labels = c("a", "b"), label_x = 0.1),
                       plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                 labels = c("c", "d"), label_x = 0.1),
...
```{r}
# Saving multi-panel plot
ggsave(file.path(out_folder, "fao_regions_perc_change_map_50-00.pdf"), device = "pdf",
       width = 14, height = 9)
```
  
## Figure 9 FAO report - FAO sustainable fisheries
Finally, we will create a choropleth map for the proportion of sustainable fisheries within FAO regions.  
  
```{r}
#Loading FAO fisheries data
fao_fisheries <- read_csv("../data/FAO_fisheries_data.csv", 
                          #Keeping only relevant columns
                          col_select = c("GEO_AREA_NAME", "TIME_PERIOD", "OBS_VALUE")) |> 
  #We will use data for FAO regions only and for 2019 only
  filter(TIME_PERIOD == 2019 & str_detect(GEO_AREA_NAME, "FAO")) |> 
  ...
  #We will remove the year column because we do not need it
  select(!TIME_PERIOD) 
...
```{r}
 ggplot()+
  geom_sf(data = fao_eez_shp_proj, fill = "white")+
  #Plotting FAO classified by ocean basin
...
maps_data_sd <- list.files(out_folder, pattern = "_perc_bio_change_data_map.csv", full.names = T) |> 
  ...
  group_by(x, y, ID_mrgd) |> 
  #Apply calculations to biases only
  summarise(across(mean00_ssp126:mean50_ssp585, 
                   #Listing statistics to be calculated
                   list(sd = sd), 
                   #Setting column names
                   .names = "{.col}_{.fn}")) |> 
...
```{r}
#Define base steps for maps
base_gg <- list(scale_fill_distiller(palette = "Blues", direction = 1, na.value = NA,
                                     limits = c(0, 100), oob = scales::oob_squish),
                geom_sf(inherit.aes = F, data = world_proj, show.legend = F),
                geom_sf(inherit.aes = F, data = eez_proj, fill = NA, colour = "#5b5b5b",
                        show.legend = F, linewidth = 0.25),
                theme_bw(),
...
#SSP1-2.6 2041-2050
p50_126 <- ggplot()+
  geom_spatraster(data = maps_data_sd_proj$mean50_ssp126_sd)+
  labs(title = "SSP1-2.6: 2041-2050")+
  base_gg

#SSP5-8.5 2041-2050
p50_585 <- ggplot()+
  geom_spatraster(data = maps_data_sd_proj$mean50_ssp585_sd)+
  labs(title = "SSP5-8.5: 2041-2050")+
  base_gg

#SSP1-2.6 2091-2100
p00_126 <- ggplot()+
  geom_spatraster(data = maps_data_sd_proj$mean00_ssp126_sd)+
  labs(title = "SSP1-2.6: 2091-2100")+
  base_gg

#SSP5-8.5 2091-2100
p00_585 <- ggplot()+
  geom_spatraster(data = maps_data_sd_proj$mean00_ssp585_sd)+
  labs(title = "SSP5-8.5: 2091-2100")+
  base_gg+
  guides(fill = guide_colorbar(title = "Ensemble standard deviation in fish biomass",
...
title <- ggdraw()+
  draw_label("Ensemble standard deviation in fish biomass")+
  theme(plot.margin = margin(0, 0, 0, 0, unit = "cm"))

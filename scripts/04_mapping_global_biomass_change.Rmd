...
```{r}
#Defining projection to be used for Pacific maps
plate_caree <- "+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=-149 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +type=crs"

#Split world in two hemispheres 
west <- world |> 
  st_crop(st_bbox(c(xmin = -180, ymin = -65, xmax = -60, ymax = 65))) |> 
  st_transform(plate_caree)
east <- world |> 
  st_crop(st_bbox(c(xmin = 120, ymin = -65, xmax = 180, ymax = 65))) |> 
  st_transform(plate_caree)
#Merge them together
pacificcent <- rbind(west, east)

#Remove variables not needed
rm(west, east)

#Getting FAO regions for the Pacific 
pacific_fao <- fao_eez_shp |> 
  filter(OCEAN == "Pacific") |> 
  st_transform(plate_caree)

#Extract data for scenario, transform to raster and reproject
pacific_data <- maps_data |> 
  filter(OCEAN == "Pacific") |> 
  #Selecting relevant columns
  select(x, y, starts_with("rel_change")) |> 
  #Converting to raster
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish),
                geom_sf(inherit.aes = F, data = pacificcent, show.legend = F),
                geom_sf(inherit.aes = F, data = pacific_fao,
                        fill = NA, colour = "#5b5b5b", show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5),
...
#Create title for 
title <- ggdraw()+
  draw_label("Mean % change in fish biomass in FAO Pacific regions between 2005-2014")+
  theme(plot.margin = margin(0, 0, 0, 0, unit = "cm"))

#Plotting everything together
pacific_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, nrow = 1,
                                        labels = c("a", "b"), label_x = 0.1),
                       plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                 labels = c("c", "d"), label_x = 0.1),
                       legend, ncol = 1, nrow = 4, 
...
```{r}
# Saving multi-panel plot
ggsave(file.path(out_folder, "pacific_perc_change_map_50-00.pdf"), device = "pdf",
       width = 10, height = 9)
```
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish),
                geom_sf(inherit.aes = F, data = world, show.legend = F),
                geom_sf(inherit.aes = F, data = fao_eez_shp[fao_eez_shp$OCEAN == "Indian",], 
                        fill = NA, colour = "#5b5b5b",
                        show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
...
#Create title for 
title <- ggdraw()+
  draw_label("Mean % change in fish biomass in Indian FAO regions between 2005-2014")+
  theme(plot.margin = margin(0, 0, 0, 0, unit = "cm"))

#Plotting everything together
indian_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, nrow = 1,
                                          labels = c("a", "b"), label_x = 0.1),
                       plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                  labels = c("c", "d"), label_x = 0.1),
                       legend, ncol = 1, nrow = 4, 
...
```{r}
# Saving multi-panel plot
ggsave(file.path(out_folder, "indian_perc_change_map_50-00.pdf"), device = "pdf",
       width = 10, height = 9)
```
...
```{r}
#Getting Antarctic continent and reprojecting
ant <- world |> 
  st_crop(st_bbox(c(xmin = -180, ymin = -90, xmax = 180, ymax = -40))) |> 
  st_transform(3976)

#FAO regions for the Southern Ocean
so_fao <- fao_eez_shp |> 
  filter(str_detect(OCEAN, "Southern")) |> 
  st_transform(3976)
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish),
                geom_sf(inherit.aes = F, data = ant, show.legend = F),
                geom_sf(inherit.aes = F, data = so_fao, 
                        fill = NA, colour = "#5b5b5b",
                        show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5), 
                      legend.position = "none"),
                coord_sf(ylim = c(-3323160.27, 4323160.27)))
...
#Create title for 
title <- ggdraw()+
  draw_label("Mean % change in fish biomass in Southern Ocean FAO regions between 2005-2014")+
  theme(plot.margin = margin(0, 0, 0, 0, unit = "cm"))

#Plotting everything together
so_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, nrow = 1,
                                          labels = c("a", "b"), label_x = 0.1),
                       plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                  labels = c("c", "d"), label_x = 0.1),
                       legend, ncol = 1, nrow = 4, 
...
```{r}
# Saving multi-panel plot
ggsave(file.path(out_folder, "southern-ocean_perc_change_map_50-00.pdf"), device = "pdf",
       width = 8, height = 9)
```
...
#Getting Antarctic continent and reprojecting
art <- world |> 
  st_crop(st_bbox(c(xmin = -180, ymin = 0, xmax = 180, ymax = 90))) |> 
  st_transform(3413) |>
  st_crop(st_bbox(c(xmin = -3314693.24, ymin = -3314693.24, 
                    xmax = 3314693.24, ymax = 3314693.24)))

#FAO regions for the Arctic
art_fao <- fao_eez_shp |> 
  filter(OCEAN == "Arctic") |> 
  st_transform(3413)
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish),
                geom_sf(inherit.aes = F, data = art, show.legend = F),
                geom_sf(inherit.aes = F, data = art_fao, 
                        fill = NA, colour = "#5b5b5b",
                        show.legend = F, linewidth = 0.25),
                theme_bw(),
                theme(axis.title = element_blank(), panel.border = element_rect(colour = NA),
...
#Create title for 
title <- ggdraw()+
  draw_label("Mean % change in fish biomass in Arctic FAO regions between 2005-2014")+
  theme(plot.margin = margin(0, 0, 0, 0, unit = "cm"))

#Plotting everything together
art_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, nrow = 1,
                                          labels = c("a", "b"), label_x = 0.1),
                       plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                  labels = c("c", "d"), label_x = 0.1),
                       legend, ncol = 1, nrow = 4, 
...
```{r}
# Saving multi-panel plot
ggsave(file.path(out_folder, "arctic_perc_change_map_50-00.pdf"), device = "pdf",
       width = 8, height = 9)
```

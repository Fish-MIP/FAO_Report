...
data_dir <- "/rd/gem/private/users/camillan/Extract_tcblog10_Data/Output/sumSize_annual/" 
...
scale_fill_custom <- function (..., alpha = 1, begin = 0, end = 1, direction = 1, 
  ...
  continuous_scale(aesthetics, scale_name = "custom", 
    palette = scales:::gradient_n_pal(c(cmocean("matter", start = 0.1, end = 0.8, direction = -1)(123),
                                        ...
                                        cmocean("deep", start = 0.1, end = 0.3)(123)), 
                                      values, space), 
...
p50_126 <- asia_data |> 
  ...
  geom_sf(inherit.aes = F, data = eez[eez$region == "Asia",], fill = NA, colour = "#5b5b5b",
          show.legend = F, linewidth = 0.25)+
  theme_bw()+
  labs(title = "SSP1-2.6: 2041-2050")+
  theme(axis.title = element_blank(), 
...
p50_126 <- africa_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  geom_sf(inherit.aes = F, data = world, show.legend = F)+
  geom_sf(inherit.aes = F, data = eez[eez$region == "Africa",], fill = NA, colour = "#5b5b5b",
          show.legend = F, linewidth = 0.25)+
  theme_bw()+
  labs(title = "SSP1-2.6: 2041-2050")+
  theme(axis.title = element_blank(), 
...
europe_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  geom_sf(inherit.aes = F, data = st_shift_longitude(world), show.legend = F)+
  # geom_sf(inherit.aes = F, data = eez[eez$region == "Europe",], fill = NA, colour = "#5b5b5b",
          show.legend = F, linewidth = 0.25)+
  theme_bw()+
  labs(title = "SSP1-2.6: 2041-2050")+
  theme(axis.title = element_blank(), 
...
p50_126 <- atlantic_data |> 
  ...
  scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                    type = scale_fill_custom, oob = scales::oob_squish)+
  geom_sf(inherit.aes = F, data = world, show.legend = F)+
  geom_sf(inherit.aes = F, data = fao_eez_shp[fao_eez_shp$OCEAN == "Atlantic",], fill = NA, colour = "#5b5b5b",
          show.legend = F, linewidth = 0.25)+
  theme_bw()+
  labs(title = "SSP1-2.6: 2041-2050")+
  theme(axis.title = element_blank(), 
...
### Figure 9 FAO report - Indian

### Figure 10 FAO report - Southern Ocean

### Figure 11 FAO report - Arctic

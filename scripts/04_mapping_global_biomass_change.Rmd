...
#EEZ boundaries
eez_proj <- read_sf(list.files(out_folder, pattern = "eez_short.shp", 
...
fao_eez_shp_proj <- read_sf(list.files(out_folder, pattern = "EEZ.shp", 
                                       recursive = T, full.names = T)) |> 
  st_transform(rob_proj)
...
Capture fisheries production data comes from the [FAO's FishStatJ tool](www.fao.org/fishery/en/statistics/software/fishstatj). 
  
```{r}
#Load fisheries data
fisheries_catch <- read_sheet("https://docs.google.com/spreadsheets/d/1_nD2TWJGUR6DStHwPMgwLm7VIoPOiDl3DKM5pa2rk64/edit#gid=162477025", 
                              sheet = 2) |> 
  ...
  mutate(country_name = case_when(str_detect(country_name, "Tanzania") ~
                                  ...
                                  str_detect(country_name, "Congo,") ~
                                    "Democratic Republic of the Congo",
                                  str_detect(country_name, "Falkland") ~
                                    "Falkland Islands (Malvinas)",
                                  str_detect(country_name, "French Southern") ~
                                    "French Southern Territories",
                                  str_detect(country_name, "Bolivia") ~ 
                                    "Bolivia (Plurinational State of)",
                                  str_detect(country_name, "Venezuela") ~
                                    "Venezuela (Bolivarian Republic of)",
                                  str_detect(country_name, "Lao") ~
                                    "Lao People's Democratic Republic",
                                  str_detect(country_name, "Korea, Dem") ~
                                    "Democratic People's Republic of Korea",
                                  str_detect(country_name, "Korea, Republic") ~
                                    "Republic of Korea",
                                  str_detect(country_name, "Iran") ~
                                    "Iran (Islamic Republic of)",
                                  str_detect(country_name, "Moldova") ~
                                    "Republic of Moldova",
                                  str_detect(country_name, "United Kingdom") ~
                                    "United Kingdom of Great Britain and Northern Ireland",
                                  T ~ country_name)) |> 
  #Select relevant columns
  select(c(country_name, x2012:x2021)) |> 
  ...
  pivot_longer(!country_name, values_to = "annual_catch", names_to = "year") |> 
...
fao_countries <- read_excel(list.files(out_folder, 
  ...
  #Matching codes world shapefile
  mutate(iso3 = case_when(iso3 == "PSE" ~ "PSX",
                          iso3 == "SSD" ~ "SDS",
                          iso3 == "ESH" ~ "SAH",
                          T ~ iso3))
...
world_proj <- world_proj |> 
  left_join(fisheries_catch |> select(!country_name), 
            by = join_by("adm0_a3" == "iso3"))
```
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8, 
                ...
                geom_sf(inherit.aes = F, 
                        data = world[world$continent != "Asia",]),
                theme_bw(),
                theme(axis.title = element_blank(), 
                      panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5), 
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8, 
                ...
                geom_sf(inherit.aes = F, data = eez_euro, fill = NA, 
                        colour = "#5b5b5b", show.legend = F, linewidth = 0.25),
                geom_sf(inherit.aes = F, 
                        data = eurocent[eurocent$continent != "Europe",]),
                theme_bw(),
                theme(axis.title = element_blank(), 
                      panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5),
...
p50_126 <- europe_data |> 
  ...
  geom_sf(inherit.aes = F, data = eurocent, 
          aes(fill = rel_change_mean50_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25)+
  labs(title = "SSP1-2.6: 2041-2050")+
  base_gg
...
p50_585 <- europe_data |> 
  ...
  geom_tile()+
  geom_sf(inherit.aes = F, data = eurocent, 
          aes(fill = rel_change_mean50_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25)+
  labs(title = "SSP5-8.5: 2041-2050")+
  base_gg
...
p00_126 <- europe_data |> 
  ...
  geom_tile()+
  geom_sf(inherit.aes = F, data = eurocent, 
          aes(fill = rel_change_mean00_ssp126), 
          colour = "#5b5b5b", linewidth = 0.25)+
  labs(title = "SSP1-2.6: 2091-2100")+
  base_gg
...
p00_585 <- europe_data |> 
  ...
  geom_tile()+
  geom_sf(inherit.aes = F, data = eurocent, 
          aes(fill = rel_change_mean00_ssp585), 
          colour = "#5b5b5b", linewidth = 0.25)+
  labs(title = "SSP5-8.5: 2091-2100")+
  base_gg+
  guides(fill = guide_colorbar(title = "% changes in human population\nand in fish biomass",
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8, 
                ...
                geom_sf(inherit.aes = F, data = eez_amer, fill = NA, 
                        colour = "#5b5b5b", show.legend = F, linewidth = 0.25),
                geom_sf(inherit.aes = F, 
                        data = amercent[str_detect(amercent$continent, 
                                                   "America", negate = T),]),
                theme_bw(),
                theme(axis.title = element_blank(), 
                      panel.border = element_rect(colour = NA),
...
#Plotting everything together
americas_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, 
                                             nrow = 1, labels = c("a", "b"), 
                                             label_x = 0.1),
                            plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                      labels = c("c", "d"), label_x = 0.1),
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8, 
                ...
                geom_sf(inherit.aes = F, data = eez_ocean, fill = NA, 
                        colour = "#5b5b5b", show.legend = F, linewidth = 0.25),
                geom_sf(inherit.aes = F, 
                        data = oceancent[oceancent$continent != "Oceania",]),
                theme_bw(),
                theme(axis.title = element_blank(), 
                      panel.border = element_rect(colour = NA),
                      plot.title = element_text(hjust = 0.5),
...
#Plotting everything together
oceania_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, 
                                            nrow = 1, labels = c("a", "b"), 
                                            label_x = 0.1),
                           plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                     labels = c("c", "d"), label_x = 0.1),
...
#Plotting everything together
fao_reg_plots <- plot_grid(title, plot_grid(p50_126, p50_585, ncol = 2, 
                                            nrow = 1, labels = c("a", "b"), 
                                            label_x = 0.1),
                           plot_grid(p00_126, p00_585, ncol = 2, nrow = 1,
                                     labels = c("c", "d"), label_x = 0.1),
...
title <- ggdraw()+
  draw_label("Ensemble variability in fish biomass change")+
  theme(plot.margin = margin(0, 0, 0, 0, unit = "cm"))

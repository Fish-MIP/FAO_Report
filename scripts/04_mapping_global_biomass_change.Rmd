...
fisheries_catch <- drive_read_string("https://drive.google.com/file/d/1adH-7AbNJln9E9_vaGcyit-czMmkGndo/view?usp=drive_link") |> 
  ...
  #Remove duplicates
  distinct() |> 
  #No information about units provided - Assuming it is in million tonnes
  #Matching scale in sample map provided by Julia
  mutate(MarCap1014 = MarCap1014*1e-6) |> 
  #Create new column with classification
  mutate(class_fish = cut(MarCap1014, 
                     breaks = c(0, .25, 1, 2, 4, 6, 20),
...
base_gg <- list(scale_fill_binned(limits = c(-50, 50), n.breaks = 8,
                ...
                geom_sf(inherit.aes = F, data = world_proj, aes(fill = class_fish),
                        show.legend = F),
                scale_fill_brewer(palette = "Blues", na.value = "#f5f5f5"),
...
p00_585 <- asia_data |> 
  ...
  guides(fill = guide_colorbar(title = "% changes in human population\nand in fish biomass",
                               title.position = "top", title.hjust = 0.5, barwidth = 25))+
  theme(legend.position = "bottom")

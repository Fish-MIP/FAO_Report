...
admin_keys <- read.csv(here("data/country_admin_key.csv")) %>%
  ...
  dplyr::select(-notes) %>%
  distinct(admin_name, iso3c_admin)
...
cmip_data_new <- read.csv(here("data/table_stats_country_admin.csv")) %>%
  left_join(admin_keys, by = c("Country_FAO" = "admin_name"))

regions_df_new <- cmip_data_new %>%
  dplyr::select(Country_FAO, iso3c_admin) %>%
  distinct(Country_FAO, iso3c_admin) %>%
  left_join(regions_df_2, by = c("iso3c_admin" = "ISO3_territory")) %>%
  filter(ISO3_country == iso3c_admin | iso3c_admin %in% c("TUR", "CPV", "CIV", "LBY", "PNG")) %>%
  ...
  filter(!(Country_FAO_new %in% c("Canary Islands (the)", "Madeira Islands (the)", "Chagos Archipelago (the)"))) %>%
...
length(unique(regions_df_new$Country_FAO)) # 149.. 
length(unique(cmip_data_new$Country_FAO)) # 149
...
## save iso3c lookup for shinyapp 
# write.csv(regions_df_new, here("data/iso3c_lookup_regions.csv"), row.names = FALSE)
...
spatial_scales <- c("countries_admin")
label_types <- c("country_name")
...
plot_df <- cmip_data_plot %>% ## filter for scenario
  filter(
         scenario == scen) %>%
  data.frame() %>%
  mutate(contnnt = ifelse(spatial_scale == "FAO_area", "High Seas", contnnt)) %>% # rename to continent high seas if it is high seas
...
# write.csv(rgn_labels_iso3c, here("scripts/outputs/iso3c_table_names.csv"), row.names = FALSE)
...
# if we are plotting iso3c labels instead of country name, specify that
if(label_type == "iso3c"){
...
rank_rgn_label <- rank_rgn_label %>% 
 ...
 g <- ggplot() + # ok so this works but losing order of data in plots for some reason
  # geom_col(data = data_2091, aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category), position = "stack", show.legend = TRUE) + # if you dont want patterns, use this geom instead
...
world_360 <- read_sf(here("data/world_360deg.shp")) %>%
...
ggsave(plot = g_legend, glue(here("outputs/circle_figs/legend_vertical.png")), height=20, width=20, units=c("in"), bg = "white") # for vertical
  # ggsave(plot = g_legend, glue(here("scripts/outputs/legend.png")), height=20, width=20, units=c("in"), bg = "white")

  

## try plotting eezs instead of admin country

eez_shp <- st_read(file.path(out_folder, "Merged_FAO_EEZ"), layer = "FAO_MajorAreas_200NM") 

test <- eez_shp %>% st_drop_geometry()

eez_shp_2 <- st_transform(eez_shp,  '+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs')

eez_key <- read.csv(file.path(out_folder, "Merged_FAO_EEZ/supporting_files/FAO-Major-Areas_EEZ_continent_keys.csv")) %>%
  mutate(figure_name = ifelse(!is.na(NAME_EN), NAME_EN, figure_name)) # REPLACE WITH PATH TO EEZ KEY

eez_shp_df <- eez_shp_2 %>% 
  left_join(eez_key)


eez_data <- cmip_data_all %>% 
  filter(spatial_scale %in% c("FAO_area", "countries"))

test <- eez_shp_2 %>% 
  left_join(eez_key) %>%
  right_join(eez_data, by = c("figure_name" = "fa_ffcl_shrt")) %>%
  rename(fa_ffcl_shrt = figure_name) %>%
    dplyr::select(colnames(eez_data)) %>%
    filter(!st_is_empty(.)) %>%
        mutate(fill_category = case_when(
    mean < -30 ~ "Decrease >30%",
    mean <= -20 & mean >= -30 ~ "Decrease 20 to 30%",
          ...
          geom_sf(aes(fill = fill_category), 
               ...
               labs(fill = "Change in fish biomass (%)")  +
                    coord_sf(crs =  '+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs') +
                           ...
                           geom_sf(data = globe_border, fill = NA, colour = "grey") +
          facet_grid(decade~scenario, switch = "y") +
          guides(fill = guide_legend(byrow = TRUE,
                              title.position = "top",
               title.hjust = 1,
               title.vjust = 1))

# ggsave(here("outputs/test.png"), bg = "white", height = 30, width = 30, units = "in", dpi = 300)

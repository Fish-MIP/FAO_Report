...
# EEZ data from Denisse ---- NOT HERE ANYMORE!!! BUT OK AS CAN USE THE PRINTED VERSION BELOW. 
...
old_key<-read_csv("data/eez_keys_camilla.csv") # for reference ## NOT HERE ANYMORE BUT OK AS YOU GOT NAMES 
head(old_key)
...
## add list name as we can use this column for country and territories names in figures. 
fao <- file.path(out_folder, "nocsDataExport_20240501-114742_English.csv") |> 
  read_csv() |> 
  #Select relevant columns
...
# print and adjust by adding missing names that were added for fao_official in previous step
filter(new_key, is.na(figure_name))
filter(new_key, is.na(ID_mrgd))

sort(new_key$fao_official)[18]
sort(new_key$fao_official)[135] # "the Republic of Colombia (Quitasueño)"
Encoding(sort(new_key$fao_official)[135])
Encoding(sort(new_key$figure_name)[47])


# print new final file 
write_csv(new_key, "data/EEZ_FAO_key_with_short_names.csv")

# load final file where you updated short names and check if special characters are printed OK.
# NEED TO REDO THSI STEP WITH FINAL DECISION - re utf simply save the csv file as csv UTF-8 on mac. 
library(readr)
final<-read.csv("data/EEZ_FAO_key_with_short_names2_utf.csv", fileEncoding = "UTF-8")


sort(final$figure_name)[46] # Colombia (Quitasue√±o) # problematic, not sure why but fixed below. Also fixed by pasting Quitasueño in the csv utf file. something happens where the situation for colombia Quitasueño is the opposite as per curacao...   
sort(final$fao_official)[134]
sort(final$figure_name)[53]
sort(final$figure_name)[56]
sort(final$figure_name)[168]
sort(final$figure_name)[172]
sort(final$figure_name)[208]

final<-final %>% 
  mutate(figure_name = ifelse(figure_name == "Colombia (Quitasue√±o)", "Colombia (Quitasueño)", figure_name))
sort(final$figure_name)[46]

## NOTE the final file was called eez_keys_camilla_new.csv before these changes. 

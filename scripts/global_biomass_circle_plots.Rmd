...
circle_theme <- theme(axis.line=element_blank(),
                     ...
                     # legend.position='bottom', # for horizontal
...
# g_legend <- cowplot::get_plot_component(g, "guide-box", return_all = TRUE)[[1]] # for vertical
# 
# ggsave(plot = g_legend, glue(here("scripts/outputs/legend_vertical.png")), height=20, width=20, units=c("in"), bg = "white") # for vertical
...
test <- eez_shp_2 %>% st_drop_geometry()
...
eez_df <- countries_shp %>%
  # eez_shp_df %>% 
 #   merge(., plot_df, by.x = c("figure_name", "fa_ffcl_shrt")) 
 right_join(plot_df, by = c("adm0_a3" = "ISO3_country")) %>%
  rename(ISO3_country = adm0_a3) %>% 
  dplyr::select(colnames(plot_df))
# %>%
  #vect() # make into spatvector for plotting

# high_seas_df <-  cmip_data_all %>%
#   filter(spatial_scale %in% c("FAO_area"),
#          decade == decade_type,
#          scenario == scen) %>%
#   left_join(eez_shp_df, by = c("fa_ffcl_shrt" = "nm_mrgd")) %>%
#   dplyr::select(colnames(eez_df)) %>%
#   st_as_sf()


# all_df <- rbind(eez_df, high_seas_df)
  ...
  mutate(mean = case_when(
    ...
    TRUE ~ mean
        ...
        p1 <- ggplot(data = eez_df) +
               # tidyterra::geom_spatvector(aes(fill = mean)) +
          geom_sf(aes(fill = mean)) + 
                 scale_fill_distiller(palette = "RdBu", direction = 1, limits = c(-69, 69), breaks = c(-60, -30, 0, 30, 60), labels = c("-60%", "-30%", "0%", "30%", "60%")) +
                 plain +
...
ggsave(plot = p1, glue(here("scripts/outputs/maps/map_{scen}_{decade_type}.png")), bg = "white")

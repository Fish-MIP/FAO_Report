...
# %>%
#  filter(!(fa_ffcl_shrt %in% c("Türkiye", "Georgia", "Romania", "Bulgaria", "Ukraine"))) # filter out black sea countries due to uncertainty  
...
## categorical fill - pattern for uncertainty. Using ggpattern is REALLY finnicky, and it took me a really long time to get the pattern stuff right... so beware. 
 ...
 g <- ggplot(data = rank_rgn_adj, aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category)) +
  ...
  ggsave(glue(here("scripts/circle_figs/bar_{scen}_{decade_type}_{spatial_type}_font7.png")), height=20, width=20, units=c("in"), bg = "white")
...
cmip_data_plot <- cmip_data_plot %>% 
  ...
  max_value <- max(abs(cmip_data_plot$new_index), na.rm = TRUE)
  mid_value = max_value/2
...
plot_df <- cmip_data_plot %>% ## filter for scenario and decade
  ...
  data.frame() %>%
  mutate(contnnt = ifelse(spatial_scale == "FAO_area", "High Seas", contnnt)) %>%
  arrange(contnnt, pos_neg, stacked_mean_total) # order by continent, increase/decrease, and mean

get_pos_neg_ranks <- plot_df %>%
  arrange(contnnt, pos_neg, -stacked_mean_total)
...
rank_rgn_label <- rank_rgn %>% 
  distinct(fa_ffcl_shrt, rgn_name_short, stacked_mean_total)
...
rank_rgn_adj <- rank_rgn_adj %>%
  ...
  arrange(contnnt, pos_neg, -stacked_mean_total, new_index)
...
all_categories_new <- c("Decrease >30%", "Increase <10%",  "Decrease 20-30%", "Increase 10-20%", "Decrease 10-20%", "Increase 20-30%", "Decrease <10%",  "Increase >30%")
 ...
 g <- ggplot() + # ok so this works but losing order of data in plots for some reason
   geom_bar(data = rank_rgn_adj %>% filter(decade == "2091-2100"), aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category), position = "stack", stat = "identity", show.legend = TRUE) +
      geom_bar(data = rank_rgn_adj %>% filter(decade == "2041-2050"), aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category), stat = "identity", position = "stack", show.legend = TRUE) +
  geom_errorbar( data = rank_rgn_adj,
     aes(x = 1,
    ymin = -end_point/4,
    ymax = end_point,
    alpha = 0),
  ...
  ggsave(glue(here("scripts/circle_figs/bar_{scen}_{spatial_type}_stacked.png")), height=20, width=20, units=c("in"), bg = "white")
...
# Sample dataframe
test <- rank_rgn_adj %>% 
  filter(fa_ffcl_shrt == "Arctic Sea") %>%
  dplyr::select(fa_ffcl_shrt, scenario, decade, index, fill_category, index ,new_index)

ggplot() + # ok grouping by scenario works for some reason
   geom_bar(data = test %>% filter(decade == "2091-2100"), aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category, group = decade, color = decade), position = "stack", stat = "identity", show.legend = TRUE) +
     geom_bar(data = test %>% filter(decade == "2041-2050"), aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category, group = decade, color = decade), position = "stack", stat = "identity", show.legend = TRUE) +
      scale_fill_manual(values = fill_colors, breaks = all_categories, labels = all_categories_new, drop = F) +  # Assign fill colors manually
  #circle_theme +
    labs(
         fill = "Average biomass change") +
...
Save a legend separately... 

```{r}
## save legend separately in case we need it post-processing
  
  rank_rgn_adj_new <- rank_rgn_adj %>% filter(!is.na(mean))
...
rank_rgn_adj_new$fill_category <- factor(rank_rgn_adj_new$fill_category, levels = all_categories)
...
all_categories_new <- c("Decrease >30%",  "Decrease 20-30%", "Decrease 10-20%",  "Decrease <10%", "Increase <10%", "Increase 10-20%", "Increase 20-30%",  "Increase >30%")
 ...
 g <- ggplot(data = rank_rgn_adj_new, aes(x = fa_ffcl_shrt, y = new_index, fill = fill_category)) +
   geom_bar(stat = "identity", show.legend = TRUE) +
      scale_fill_manual(values = fill_colors, breaks = all_categories, labels = all_categories_new, drop = F) +  # Assign fill colors manually
  circle_theme +
    labs(
...
g_legend <- cowplot::get_plot_component(g, "guide-box", return_all = TRUE)

ggdraw(g_legend)

ggsave(plot = g_legend, here("scripts/circle_figs/legend.png"), dpi = 300, height = 8, width =10)

...
if(spatial_type %in% c("countries", "countries_high_seas")){
  ...
  ggsave(plot = g, glue(here("scripts/outputs/{spatial_type}/bar_{scen}_{label_type}_stacked.png")), height=20, width=20, units=c("in"), bg = "white")
...
Make map of mean change and save legend 
...
eez_df$decade_clean <- factor(eez_df$decade_clean, levels = c("Mid-century", "End of century"))
        ...
        ggsave(plot = p1, glue(here("scripts/test/map_all.png")), bg = "white", height = 20, width = 20, units = "in")
...
ggsave(plot = g_legend, glue(here("scripts/test/legend_vertical.png")), height=20, width=20, units=c("in"), bg = "white") # for vertical
...
# scen = "ssp585"
    ...
    scen_plot <- data.frame(scenario = c("ssp126", "ssp585")) %>%
      mutate(scen_clean = case_when(
        scenario == "ssp126" ~ "SSP1-2.6",
        scenario == "ssp585" ~ "SSP5-8.5"
      )) %>%
...
rank_rgn <- get_pos_neg_ranks %>%
  ...
  rename(index = mean) %>%
     mutate(rgn_name_short = fa_ffcl_shrt, 
         ...
         rgn_name_short = gsub("Federated States", "Fed. States", rgn_name_short),
         rgn_name_short = gsub("United Rep of Tanzania", "Tanzania", rgn_name_short)) %>%
  mutate(rgn_name_short = ifelse(rgn_name_short == "Netherlands (Kingdom of the)", "Netherlands", rgn_name_short)) %>%
    mutate(fill_category = case_when(
    index < -30 ~ "Decrease >30%",
    index <= -20 & index >= -30 ~ "Decrease 20 to 30%",
    index <= -10 & index > -20 ~ "Decrease 10 to 20%",
...
rank_rgn$uncertain <- factor(rank_rgn$uncertain, c("yes", "no")) # order the sd uncertainty factors
...
all_categories <- c("Decrease >30%",  "Decrease 20 to 30%", "Decrease 10 to 20%", "Decrease <10%", "Increase <10%", "Increase 10 to 20%", "Increase 20 to 30%",   "Increase >30%")
rank_rgn$fill_category <- factor(rank_rgn$fill_category, levels = all_categories)
...
data_2041 <- rank_rgn %>%
  filter(decade %in% c("2041-2050"))

data_2091 <- rank_rgn %>% 
  filter(decade %in% c("2091-2100")) %>%
  arrange(contnnt, -arrange_mean)

rgn_arrange <- data_2091 %>%
  distinct(rgn_name_short)

data_2091$rgn_name_short <- factor(data_2091$rgn_name_short, levels = c(rgn_arrange$rgn_name_short))

data_2091$scen_clean <- factor(data_2091$scen_clean, levels = c("High Emissions: SSP5-8.5", "Low Emissions: SSP1-2.6"))
data_2041$scen_clean <- factor(data_2041$scen_clean, levels = c("High Emissions: SSP5-8.5", "Low Emissions: SSP1-2.6"))

  rgn_shift <- data.frame(
                         contnnt = rank_pos_neg$contnnt,
                         name_x = c(25, 55, 90, 125, 145),
...
# specify theme
circle_theme <- theme(axis.text.x = element_text(size = 16),
                      axis.text.y = element_text(size = 14),
                      axis.ticks=element_blank(),
                      axis.title.x=element_blank(),
                      axis.title.y=element_blank(),
                      legend.position='right', # for vertical
                      panel.background=element_blank(),
                      panel.border=element_blank(),
                      panel.grid.major=element_blank(),
                      panel.grid.minor=element_blank(),
 ...
 g <- ggplot() + # ok so this works but losing order of data in plots for some reason
   geom_col(data = data_2091, aes(x = rgn_name_short, y = new_index, fill = fill_category), position = "stack", show.legend = TRUE) +
  geom_col(data = data_2041, aes(x = rgn_name_short, y = new_index, fill = fill_category), position = "stack", show.legend = FALSE) +
   geom_errorbar(data = data_2041,
                 aes(x = rgn_name_short,
                     ymax = new_index,
                     ymin = new_index), color = "white", size = 0.4) + 
    geom_segment(data = data_2091, 
        x = 0,
    ...
    guides(alpha = "none",
            pattern = "none",
           fill = guide_legend( # for vertical legend
               title.position = "top",
               title.hjust = 1,
               title.vjust = 1,
               ncol = 1,
...
ggsave(plot = g, here("scripts/test/test.png"), bg = "white", height=22, width=20, units=c("in"))
...
if(spatial_type %in% c("countries", "countries_high_seas")){
  ...
  ggsave(glue(here("scripts/outputs/{spatial_type}/non_stacked/bar_{scen}_{decade_type}_pattern.png")), height=20, width=20, units=c("in"), bg = "white") # save as png
...
geom_text(
  ...
  ggsave(glue(here("scripts/outputs/{spatial_type}/non_stacked/bar_{scen}_{decade_type}.png")), height=20, width=20, units=c("in"), bg = "white")

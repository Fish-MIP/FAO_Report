...
stats<-all_scale_data_by_model %>% 
  group_by(fa_ffcl_shrt, mem, esm, scenario, decade, spatial_scale) %>% 
  ...
  ungroup() %>% 
  group_by(fa_ffcl_shrt, scenario, decade, spatial_scale) %>% 
  summarise(
...
top_7<-global_bio |> 
  ...
  base_gg+
  #Plotting by country
  facet_wrap(~Cnt_FAO, scales = "free_y", nrow = 8)
top_7
```
...
png("figures/top_7_perc_change.png", width=6, height= 12, units="in",res=1200)
top_7
dev.off()
...
asia<-count_bio |> 
  ...
  facet_wrap(~fa_ffcl_shrt, scales = "free_y", ncol = 5,
             ...
             labeller = labeller(fa_ffcl_shrt = label_wrap_gen(20)))
asia
```
...
png("figures/asia_perc_change.png", width=10, height= 14.35, units="in",res=1200)
asia
dev.off()
...
png("figures/africa_perc_change.png", width=10, height= 14.35, units="in",res=1200)
africa
dev.off()
```
...
png("figures/europe_perc_change.png", width=12, height= 16, units="in",res=1200)
europe
dev.off()
...
png("figures/americas_perc_change.png", width=12, height= 17.22, units="in",res=1200)
americas
dev.off()
```
...
png("figures/oceania_perc_change.png", width=12, height= 9, units="in",res=1200)
oceania
dev.off()
```
...
png("figures/fao_regions_perc_change.png", width=10, height= 6, units="in",res=1200)
oceania
dev.off()
```

...
global_sum2<-global_sum %>% 
  ...
  mutate(tcb_unfished_perc = ((tcb_unfished-ref_unfished$mean_tcb_unfished)/ref_unfished$mean_tcb_unfished)) 

# multiply??  
# multiply the catches by the % change in exploitable fish biomass 
# multiply the % change in exploitable fish biomass by the peak of the catches. 

global_sum2<-global_sum2 %>%
  mutate(tc_potential = tc*tcb_unfished_perc) %>%
  pivot_longer(!Year, names_to = "variable", values_to = "value") %>% 
  mutate(type = ifelse(variable %in% c("tc", "tc_potential"), "catches", "biomass"))

ggplot(filter(global_sum2, variable != "tcb_unfished_perc"), aes(x = Year, y = value, group = variable, color = variable))+
  geom_line()+

...
# Base folder for project (both inputs nad outputs)
base_folder <- "/rd/gem/private/users/camillan/FAO_Report/Merged_FAO_EEZ"
...
```{r}
fao_ema <- file.path(base_folder, "countries_un_fromEmmanuelShapeFiles_cleaned.csv") |> 
  read.csv() |>
...
# 3 left join to keep only list of country code as per 1 and add country names as per 2
...
length(duplicates)
duplicates[1] # no official name # only coming up when using read.csv() instead of read_csv()
duplicates[2] # Au Ashmore - not in eez data 
duplicates[3] # Saba etc 
duplicates[4] # Clipperton 
...
fao_final <- fao_ema |> # Emmanuel + Stefania
  full_join(fao_st)

fao_final <- fao_final |> # + website
  full_join(fao) |> 
...
new_key<- file.path(base_folder,"EEZ_FAO_key.csv") %>% 
  ...
  # remove EEZs not included in FAO data and with no information on the country they belong to as per "assumptions" files 
...
# e.g. fao_official: the Federative Republic of Brazil (Trindade) -> List name: Brazil (Trindade)

# small adjustments: 
# the United States Minor Outlying Islands (Howland and Baker Islands) -> Howland and Baker Islands
# the Commonwealth of the Northern Mariana Islands -> USA as sovreign country

# special characters 
sort(new_key$fao_official)[18] # "Cura\x8dao"
sort(new_key$fao_official)[47] # "R\x8eunion"
sort(new_key$fao_official)[49] # "Saint Barth\x8elemy"
sort(new_key$fao_official)[135] # "the Republic of Colombia (Quitasueño)" # NOTE: changed to Quitasueño in csv before saving
sort(new_key$fao_official)[141] # "the Republic of C\x99te d'Ivoire"   
sort(new_key$fao_official)[196] # "the Republic of T\x9frkiye" 

...
#Filtering data
bio_data |> 
  filter(region == "Europe" & str_detect(nm_mrgd, "Overlapping|Clipperton", 
                                         negate = T)) |> 
  mutate(nm_mrgd = str_replace(nm_mrgd, "Exclusive.*one", "EEZ")) |> 
  ggplot(aes(x = year, y = mean_change, colour = scenario))+
  base_gg+
...
#Filtering data
bio_data |> 
  filter(str_detect(region, "America") & str_detect(nm_mrgd, "Overlapping", 
                                                    negate = T)
...
#Filtering data
bio_data |> 
  filter(region == "Oceania" & str_detect(SOVEREIGN1, "France|United", 
                                          negate = T)) |>
  mutate(nm_mrgd = str_replace(nm_mrgd, "Exclusive.*one", "EEZ")) |> 
  ggplot(aes(x = year, y = mean_change, colour = scenario))+
  base_gg+

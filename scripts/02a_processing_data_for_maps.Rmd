...
#Shapefile containing EEZ and FAO boundaries
fao_eez <- list.files(out_folder, "FAO_MajorAreas_200NM.shp", recursive = T, 
                      full.names = T) |> 
  read_sf()
```
...
choro_data <- read_csv("data/ensemble_perc_bio_change_data_map.csv") |> 
  ...
  summarise(across(starts_with("rel_change"), 
                   list(weighted_mean = ~weighted.mean(.x, cell_area, 
                                                       na.rm = T)), 
                   .names = "{.col}"))

#Joining weighted means to FAO-EEZ shapefile
choro_data <- fao_eez |> 
  left_join(choro_data, join_by(ID_mrgd))

#Save shapefile
choro_data |> 
  write_sf("data/fao_eez_bio_change_choro.shp")
```

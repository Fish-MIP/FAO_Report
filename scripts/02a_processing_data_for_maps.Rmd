...
#Shorten layer names before saving to disk
lyr_names <- names(choro_data) |> 
  str_remove("rel_change_") |> 
  str_remove("_mean$")

#Saving each layer separately to record layer names correctly
for(i in 1:nlyr(choro_data)){
  writeRaster(choro_data[[i]], 
              file.path("data", str_c("global_choro_", lyr_names[i], ".tif")),
              overwrite = T)
}
```

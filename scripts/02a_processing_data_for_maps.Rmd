...
global_files <- list.files("/rd/gem/private/users/camillan/Extract_tcblog10_Data/Output/sumSize_annual/sizeConsidered10g_100kg/", # needs full path as there are different/older versions that get picked up otherwise 
                           pattern = "global_10g-100kg.rds", 
...
#Adding LME metadata
lme <- read_csv(file.path(out_folder, "Merged_FAO_EEZ/gfdl-esm4_60arcmin_global_FAO-LME.csv")) |>
...
maps_data |> 
  ...
  write_csv(file.path(out_folder, "ensemble/ensemble_perc_bio_change_data_map.csv"))

```
    
## Calculated spatially weighted percentage change for FishMIP ensemble 
  
```{r}
choro_data <- read_csv(file.path(out_folder, "ensemble/ensemble_perc_bio_change_data_map.csv")) |> 
...
#Saving result
choro_data |> 
  write_csv(file.path(out_folder, "global_choro_maps_data.csv"))
```
...
#Save results 
pop_change |> 
  write_csv(file.path(out_folder, "world_population_change.csv"))
```
...
for(m in members){
  ...
  area_grid <- area_grid |> 
    # drop_na(OCEAN) |> # CN not available anymore
...
#Save ensemble data
fao_map_data |> 
  write_csv(file.path(out_folder, "ensemble/ensemble_perc_bio_change_fao_map.csv"))
```
...
#Save results
maps_data_sd |> 
  write_csv(file.path(out_folder,"ensemble/ensemble_sd_bio_change_data_map.csv"))
```
...
#Saving result
maps_data_cv |> 
  write_csv(file.path(out_folder,"ensemble/ensemble_cv_bio_change_data_map.csv"))
```
...
#Save results
maps_data_agreement |> 
  write_csv(file.path(out_folder,"ensemble/ensemble_model_agreement.csv"))
```

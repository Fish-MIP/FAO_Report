...
ggsave(filename = file.path("outputs/", "fao_eez_global_map.pdf"), main,
       device = "pdf", width = 14, height = 9)
```
...
```{r}
#Load fisheries data
fisheries_catch <- read_csv(file.path(out_folder, "world_mean_annual_catch.csv")) |> 
...
maps_data <- read_csv(file.path(out_folder,"ensemble/ensemble_perc_bio_change_data_map.csv"))
...
ggsave(file.path("outputs/", "global_perc_change_map_50-00.pdf"), all_plots,
       device = "pdf", width = 14, height = 8)
```
...
ggsave(file.path("outputs/", "asia_perc_change_map_50-00.pdf"), asia_plots, 
       device = "pdf", width = 14, height = 8)
```
...
ggsave(file.path("outputs/", "americas_perc_change_map_50-00.pdf"), 
       americas_plots, device = "pdf", width = 14, height = 9)
...
ggsave(file.path("outputs/", "europe_perc_change_map_50-00.pdf"),
       europe_plots, device = "pdf", width = 14, height = 8)
```
...
ggsave(file.path("outputs/", "africa_perc_change_map_50-00.pdf"),
       africa_plots, device = "pdf", width = 14, height = 9)
```
...
ggsave(file.path("outputs/", "oceania_perc_change_map_50-00.pdf"),
       oceania_plots, device = "pdf", width = 14, height = 8)
```
...
maps_data <- read_csv(file.path(out_folder,"ensemble/ensemble_perc_bio_change_fao_map.csv"))
...
ggsave(file.path("outputs/", "fao_regions_perc_change_map_50-00.pdf"),
       fao_reg_plots, device = "pdf", width = 14, height = 9)
```
...
maps_data_sd <- read_csv(file.path(out_folder,"ensemble/ensemble_sd_bio_change_data_map.csv"))
...
```{r}
# Listing all relevant files to calculate biomass projections
maps_data_agreement_proj <- read_csv(file.path(out_folder,"ensemble/ensemble_model_agreement.csv")) |>   
  #Transform to multi-layer raster
  rast(type = "xyz", crs = "epsg:4326") |>
  #Reproject to Robinson
  project(y = rob_proj)
...
ggsave(file.path("outputs/", "agreement_map_00.pdf"), all_plots, 
       device = "pdf", width = 14, height = 13.5)
```

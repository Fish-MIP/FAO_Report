...
### DO NOT REMOVE - THE APPPROACH ABOVE DOES NOT WORK FOR ME AS EXPLAIN IN MY EMAIL. 
...
# leg_land
# zeroGrob[NULL] # NO LEGEND

# MY WORKAROUND:
leg_land = cowplot::get_plot_component(land, 'guide-box-bottom', return_all = TRUE)
# cowplot::ggdraw(leg_land) 
...
### DO NOT REMOVE 
leg_fish = cowplot::get_plot_component(fish, 'guide-box-bottom', return_all = TRUE)

#SSP1-2.6 2041-2050
p50_126_change <- ggplot()+ #### KEEP THE NEW NAMES (_change) FOR THESE 4 FIGS AS I NEED TO RE-USE THEM LATER AND DO NOT WANT TO OVERWRITE THEM 
  geom_spatraster(data = maps_data_proj$rel_change_mean50_ssp126_mean)+
...
all_plots <- plot_grid(plot_grid(p50_126_change, p50_585_change, ncol = 2, nrow = 1,
                                 labels = c("a", "b"), label_x = 0.1),
                       plot_grid(p00_126_change, p00_585_change, ncol = 2, nrow = 1,
...
## DO NOT REMOVE - I CANNOT DOWLOAD FROM GEM RIGHT NOW AND NEED EASY ACCESS TO PLOTS 
# I AM NOT PUSHING FIGURES ON GITHUB SO NOT A BIG DEAL TO KEEP THIS IN FOR NOW. COMMENT OUT IF YOU WANT.
ggsave("figures/global_perc_change_map_50-00.pdf", all_plots,
       device = "pdf", width = 14, height = 8)
...
#### NO continent COLUMN IN THIS DATAFRAME, REPLACED WITH contnnt BUT CODE BELOW STILL DOES NOT WORK. SAME FOR OTHER "CONTINENT" MAPS
...
#SSP1-2.6 2041-2050
p50_126_SD <- ggplot()+ #### KEEP THE NEW NAMES (_SD) FOR THESE 4 FIGS AS I NEED TO RE-USE THEM LATER AND DO NOT WANT TO OVERWRITE THEM 
...
#SSP5-8.5 2091-2100
p00_585_SD <- ggplot()+ # SAME AS FOR FIGURE ABOVE 
...
#Get legend for land
# legend <- get_legend(p00_585_SD)

## DO NOT REMOVE 
legend_sd = cowplot::get_plot_component(p00_585_SD, 'guide-box-bottom', return_all = TRUE)
...
## DO NOT REMOVE 
legend_cv = cowplot::get_plot_component(p00_585_CV, 'guide-box-bottom', return_all = TRUE)
...
maps_data_agreement <- list.files(out_folder, 
  ...
  summarise(n_model = sum(!is.na(value)), # numbers of models for which info is available 
            n_positive = sum(value >= 0, na.rm = T), # number of non-negative estimates 
            n_negative = sum(value < 0, na.rm = T), # number of negative estimates
...
#Get legend for land
legend_agreement = cowplot::get_plot_component(p00_585_agreement, 'guide-box-bottom', return_all = TRUE)
...
#Plotting everything together
## NOT QUITE RIGHT- NEEDS FIXING
all_plots <- plot_grid(plot_grid(plot_grid(p00_126_change, p00_585_change, ncol = 2, nrow = 1,
                                 labels = c("a", "b"), label_x = 0.1),
                       plot_grid(leg_land, leg_fish, nrow = 1, ncol = 2),
                       ncol = 1, nrow = 2, rel_heights = c(1, 0.4)),
                       plot_grid(plot_grid(p00_126_SD, p00_585_SD, ncol = 2, nrow = 1,
                                 labels = c("c", "d"), label_x = 0.1),
                       legend_sd, ncol = 1, nrow = 2,
                       rel_heights = c(1, 0.4)), 
                       plot_grid(plot_grid(p00_126_agreement, p00_585_agreement, ncol = 2, nrow = 1,
                                 labels = c("e", "f"), label_x = 0.1),
                       legend_agreement, ncol = 1, nrow = 3,
                       rel_heights = c(1, 0.4)),
...
ggsave("figures/agreement_map_00.pdf", all_plots,

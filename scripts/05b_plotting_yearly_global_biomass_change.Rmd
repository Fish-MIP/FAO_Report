...
#EEZ keys
eez_keys <- read_csv(list.files(out_folder, 
                           pattern = "EEZ_keys_updated.csv", 
                           full.names = T, recursive = T)) |> 
  #Keeping relevant columns
  select(ID_mrgd, Name_EEZ, region, Official_Name_En_Stefania_NEW) |> 
  #Removing jointly managed areas and overlapping claims
...
area_grid <- rast(list.files(out_folder, pattern = "^area_1deg.nc", 
  ...
  left_join(eez_keys, by = join_by(mask == ID_mrgd)) |> 
  left_join(fao, by = join_by(mask == ID_mrgd)) |> 
  rename("ID_mrgd" = "mask")

#Remove original mask
rm(fao_eez_mask, fao, eez_keys)
...
fao_eez_keys <-  area_grid |>
  select(ID_mrgd:OCEAN) |> 
  distinct()
...
bio_data <- bio_data |> 
  ...
  left_join(fao_eez_keys, by = "ID_mrgd") |> 
  ungroup()

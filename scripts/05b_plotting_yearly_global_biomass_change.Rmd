...
ggplot(bio_dataOCD, aes(x=fao_official, y=mean_change, group=scenario, color=scenario)) + 
...
ggsave(filename = "figures/FAOarea_global_trends_ODC.pdf",
       device = "pdf", width = 7, height = 5)

unique(bio_dataOCD$fao_official)
filter(bio_dataOCD, fao_official == "Atlantic, Antarctic") # something strange here - this is +5% in maps but maps and trends values for aggregated regions should be exactly the same. For maps, were grid cells % values averaged across models instead of using grid cell biomass to calculate one percentage value for the whole region and then model-specific values averadged across models?   
